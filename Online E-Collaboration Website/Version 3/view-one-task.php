...
if( isset($_GET['taskPart_status']) && isset($_GET['task_id'])) {
    ...
    $check_reject_status = mysqli_query($link, 'SELECT * FROM taskDetails_T WHERE taskPart_status = 2');
    if ($check_reject_status->num_rows > 0) {
        $status_id = 3;
    }
    else {
            ...
            <?php while($task = mysqli_fetch_object($task_results) )  { ?>
                <div class="box">
                    ...
                    <form action="./view-one-task.php" method="GET">
                        <!-- Radio Buttons -->
                        <p>Current Task Status: <?php echo $current_statusPart_status; ?></p>
                        <br>
